...
public final class SpawnAnchorResolver {
        ...
        for (AnchorCandidate candidate : candidates) {
            ...
            if (!validator.isSafe(level, profile, spawnPos)) {
                ...
                if (safeNearby.isEmpty()) {
                    CustomizablePlayerSpawnCommon.LOGGER.warn("Spawn anchor {} at {} is unsafe and no nearby safe fallback was found.", candidate.source(), spawnPos);
                    continue;
                }
    ...
    private Optional<Block> markerBlock(SpawnProfile profile) {
        ...
        if (markerBlockId == null) {
            CustomizablePlayerSpawnCommon.LOGGER.error("Configured anchor.markerBlock {} is not a valid resource location.", markerBlockName);
            return Optional.empty();
        }

        Optional<Block> block = BuiltInRegistries.BLOCK.getOptional(markerBlockId);
        if (block.isEmpty() || block.get() == Blocks.AIR) {
            CustomizablePlayerSpawnCommon.LOGGER.error("Configured anchor.markerBlock {} does not exist as a loaded block.", markerBlockId);
            return Optional.empty();
        }
