...
public final class SpawnProfileValidator {
    ...
    public static SpawnProfileValidationResult validate(SpawnProfile profile) {
        ...
        if (profile.spawnPolicy().maxInstances() < 0) {
            errors.add("spawnPolicy.maxInstances must be 0 or greater");
        }

        if (profile.spawnPolicy().minDistanceBetweenInstances() < 0) {
            errors.add("spawnPolicy.minDistanceBetweenInstances must be 0 or greater");
        }

        if (profile.protection().padding() < 0) {
            errors.add("protection.padding must be 0 or greater");
        }
